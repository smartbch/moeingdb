...
TEST_CASE( "my_map's iterator is tested", "[bigmap]") {
	...
	m.insert(4, 0x1, 0x1);
	m.insert(6, 0x2, 0x2);
	auto it = m.get_iterator(0, 0x0, 7, 0x0);
	...
	it = m.get_iterator(2, 0x12, 2, 0x13);
	REQUIRE(it.valid() == true);
	REQUIRE(it.curr_idx() == 2); REQUIRE(it.key() == 0x12); REQUIRE(it.value() == 0x34); it.next(); 
	REQUIRE(it.valid() == false);
	it = m.get_iterator(2, 0x15, 3, 0x13);
	REQUIRE(it.valid() == false);
	it = m.get_iterator(2, 0x15, 5, 0x13);
	REQUIRE(it.curr_idx() == 4); REQUIRE(it.key() == 0x1); REQUIRE(it.value() == 0x1); it.next();
	REQUIRE(it.valid() == false);
	it = m.get_iterator(3, 0x1, 4, 0x0);
	REQUIRE(it.valid() == false);
	it = m.get_iterator(3, 0x1, 4, 0x2);
	REQUIRE(it.curr_idx() == 4); REQUIRE(it.key() == 0x1); REQUIRE(it.value() == 0x1); it.next();
	REQUIRE(it.valid() == false);
	it = m.get_iterator(5, 0x9, 6, 0x2);
	REQUIRE(it.valid() == false);
	it = m.get_iterator(5, 0x9, 6, 0x3);
	REQUIRE(it.curr_idx() == 6); REQUIRE(it.key() == 0x2); REQUIRE(it.value() == 0x2); it.next();
	it = m.get_iterator(6, 0x9, 6, 0xA);
	REQUIRE(it.valid() == false);
	it = m.get_iterator(7, 0x9, 7, 0xA);
	REQUIRE(it.valid() == false);
}
