...
type BigArray struct {
	segMap map[int64]*[SegLen][6]byte
}

func NewBigArray() *BigArray {
	return &BigArray{
		segMap: make(map[int64]*[SegLen][6]byte),
	}
}

func (ba *BigArray) Set(height, pos int64) {
	seg, ok := ba.segMap[height>>SegBits]
	if !ok {
		seg = &[SegLen][6]byte{}
		ba.segMap[height>>SegBits] = seg
	}
	var buf [8]byte
...
func (ba *BigArray) Get(height int64) int64 {
	seg, ok := ba.segMap[height>>SegBits]
	if !ok {
		return 0
	}
...
func NewSyncDB(path string) *SyncDB {
	...
	db := &SyncDB{hpfile: hpfile, index: NewBigArray()}
